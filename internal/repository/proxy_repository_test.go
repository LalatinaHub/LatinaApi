...
func TestProxyRepository_GetProxiesByFilter_MultiValue(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	rows := sqlmock.NewRows(cols).
		AddRow(
			1, "proxy.example.com", "1.1.1.1", 443, "uuid-1", "pass", "auto", 0,
			"", "", "", "example.com", true, "ws", "/ws", "",
			true, "example.com", "Asia-Node-1", "cdn", "SG", "Asia", "Cloudflare", "vless", "vless://raw",
		)

	mock.ExpectQuery("SELECT (.+) FROM proxies WHERE (.+) ORDER BY RANDOM\\(\\) LIMIT \\?").
		WithArgs("vmess", "vless", "trojan", "asia", "cdn", "sni", 10).
		WillReturnRows(rows)
	...
	require.NoError(t, err)
	require.Len(t, proxies, 1)
	assert.Equal(t, "Asia-Node-1", proxies[0].Remark)
	assert.Equal(t, "vless", proxies[0].VPN)
